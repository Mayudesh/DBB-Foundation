...
001900     SELECT TEMP-IP-WORK-FILE ASSIGN TO UT-S-IPFILE.
...
004500*
004600 FD  TEMP-IP-WORK-FILE
002500     RECORDING MODE IS F
004700     RECORD CONTAINS 80 CHARACTERS
003500     BLOCK CONTAINS 0 RECORDS
004800     DATA RECORD IS TEMP-IP-WORK-REC.
004900 01  TEMP-IP-WORK-REC.
...
008600     05  OMF-DEDUCTIBLE                  PIC  9(5).
008600     05  OMF-MEDID                       PIC  X(8).
008700*
...
009900*
       01  PERSON-MASTER-RECORD.
           ...
           03  PERSON-NAME.
               05  PERSON-FIRST-NAME          PIC X(12).
               05  PERSON-LAST-NAME           PIC X(15).
           03  PERSON-STREET-ADDRESS          PIC X(16).
           03  PERSON-CITY-ADDRESS            PIC X(12).
           03  PERSON-STATE-ADDRESS           PIC X(02).
           03  PERSON-SALARY                  PIC S9(07) COMP-3.
           03  FILLER                         PIC X(09).
       01  DCL-PERSONFL.
           03 PERSON-NUMBER                  PIC X(10).
...
016700     05  DL-PAY-AMT-OUT                  PIC  ZZ,ZZZ,ZZ9.99-.
           05  DL-DEDUCTIBLE                   PIC  ZZ,ZZ9.99.
016800     05  FILLER                          PIC  X(10) VALUE SPACE.
016900*
...
*****************************************************************
      **   COPYLIB FOR PERSONNEL DATABASE                            **
      *****************************************************************
...
026700     05  AC-DEDUCTIBLE            PIC S9(3)    COMP-3 VALUE ZERO.
026800*
...
027500     05  RC-ERR-MSG               PIC X(35).
           05  WS-DEDUCTIBLE            PIC S9(4) COMP VALUE 500.
           05  WS-MED-ID                PIC X(8).
           05  WS-TEMP-DED              PIC S9(4) COMP-3.
027600*
027700 01  SWITCHES.
...
029400 000-MAINLINE.
029500     PERFORM 100-INITIALIZE.
029600     PERFORM 200-PROCESS.
029700     PERFORM 300-TERMINATION.
029800     MOVE ZERO TO RETURN-CODE.
           CALL "CNTRLBRK" USING PERSON-MASTER-RECORD, WS-OLD-MF-REC.
           CALL "RPT1" USING WS-PROD-LINE-REC.
029900     GOBACK.
030200*
       100-INITIALIZE.
...
030900          INPUT PROD-LINE-FILE-IN
                INPUT TEMP-IP-WORK-FILE
031000         OUTPUT NEW-PARTS-MF-OUT
031100         OUTPUT REPORT-FILE-OUT.
031200     PERFORM 105-READ-PROD.
           MOVE 'ANKUR' TO PERSON-FIRST-NAME of PERSON-MASTER-RECORD.
           MOVE 'ANKUR' TO PERSON-FIRST-NAME of PERSON-MASTER-RECORD.
031300     IF EOF-PROD THEN
031400         DISPLAY '*** PROD FILE EMPTY ***'
031500         GOBACK.
031600     SET PROD-NDX TO 1.
031700     PERFORM 110-LOAD-PROD-TABLE
...
032700     PERFORM 229-READ-ACTIVITY.
032800     IF EOF-ACT THEN
032900         DISPLAY '*** DATA FILE EMPTY ***'
033000         GOBACK.
033100     PERFORM 278-HEADINGS.
033400*
...
034800     PERFORM 105-READ-PROD.
035100*
035200 200-PROCESS.
035300     PERFORM 220-INPUT-PROCESS.
035600     PERFORM 250-OUTPUT-PROCESS.
           CALL 'CNTRLBRK' USING  WS-OLD-MF-REC.
036100*
036200 220-INPUT-PROCESS.
036300     PERFORM 222-SELECT-RECS
036400         UNTIL EOF-ACT.
036600*
036700 222-SELECT-RECS.
036800     PERFORM 230-EDITS
036900     IF RECORD-VALID THEN
037000         PERFORM 232-LOOKUP-MODEL
037100         IF MODEL-FOUND THEN
037200             PERFORM 233-PRE-OUT-REC
037300             PERFORM 240-WRITE-ACTIVITY-REC
037400         ELSE
...
038000     PERFORM 229-READ-ACTIVITY.
038300*
...
042600*
042700 240-WRITE-ACTIVITY-REC.
042800*     WRITE TEMP-IP-WORK-REC.
...
043300*
043400
043500 250-OUTPUT-PROCESS.
043600     PERFORM 280-READ-ACTIVITY-RECORD.
043700     PERFORM 285-READ-OLD-MASTER.
043800     PERFORM 255-PROCESS-MF
...
044600         PERFORM 260-UPDATE-OLD-MASTER
044700         PERFORM 265-PREP-PRINT-LINE
...
045500             PERFORM 262-CREATE-NEW-MF
           IF AF-PARTNAME = OMF-PARTNAME
           THEN
                PERFORM 262-CREATE-NEW-MF
           ELSE
                PERFORM 262-CREATE-NEW-MF
           END-IF.
045800*
...
047600     PERFORM 276-LOOKUP-MONTH.
047900*
...
048700     MOVE AF-PARTNO     TO OMF-PARTNO.
           MOVE ZERO          TO OMF-DEDUCTIBLE.
           MOVE SPACES        TO OMF-MEDID.
049000*
049100 265-PREP-PRINT-LINE.
049200     IF RC-LINE-COUNT > RC-LINES-PER-PAGE THEN
049300         PERFORM 278-HEADINGS.
...
052000     MOVE AC-TOD-PAY-AMT   TO TL-PAY-AMT-OUT.
           MOVE AC-DEDUCTIBLE    TO WS-DEDUCTIBLE.
052100     MOVE OMF-MON          TO WS-MON.
052200     MOVE OMF-YR           TO WS-YR.
052300     PERFORM 276-LOOKUP-MONTH.
...
057600*
057700 280-READ-ACTIVITY-RECORD.
057800     READ TEMP-IP-WORK-FILE
...
059400         MOVE OMF-DEDUCTIBLE  TO AC-DEDUCTIBLE.
...
060500         MOVE OMF-PAY-AMT TO AC-TOD-PAY-AMT
      *         MOVE OMF-DEDUCTIBLE TO WS-DEDUCTIBLE.
               MOVE OMF-MEDID  TO MEDICATION-ID.
060800*
...
061600     PERFORM 275-PART-TOTALS.
061700     PERFORM 310-GRAND-TOTALS.
...
063900     DISPLAY 'WRITED ACTIVITY  ' RC-MSG-OUT.
064000     MOVE AC-RECS-RET-SRT TO RC-MSG-OUT.
064100     DISPLAY 'READED ACTIVITY  ' RC-MSG-OUT.
064200     DISPLAY ' '.
...
066300         AFTER ADVANCING 2.
      ...
      ******** EXEC SQL to get info from DB2
           ...
           MOVE WS-MED-ID TO MEDICATION-ID IN  DCLMEDICATION.

           MOVE WS-DEDUCTIBLE TO DEDUCTIBLE IN DCLHEALTH-PLAN.
           ...
           :PLAN-COMMENTS
              ...
              AND DEDUCTIBLE >  :DCLHEALTH-PLAN.DEDUCTIBLE
           END-EXEC.
           ...
           MOVE DEDUCTIBLE IN DCLHEALTH-PLAN
               TO DEDUCTIBLE IN DCLDIAG-CODES.

           MOVE DEDUCTIBLE IN DCLHEALTH-PLAN TO DL-DEDUCTIBLE.

      ****** CHECK FOR VALID DIAGNOSTIC CODE
           EXEC SQL
               UPDATE DDS0001.DIAG_CODES
                 SET DEDUCTIBLE = :DCLDIAG-CODES.DEDUCTIBLE
