      ...
      *****************************************************************
       ...
       COPY ABENDREC.
      * COPY EQUIP.
            EXEC SQL INCLUDE EQUIP END-EXEC.
       COPY DETAIL.
      ******************************************************************
      ...
      *    10 PHARMACY-COST                  PIC S9(3)V99 COMP-3.
       ...
       01  DEBUG-ABEND-FIELDS.
            10  A00-X    PIC  X(008) VALUE '00X '.
            10  A00-RECORD-SEQ REDEFINES A00-X   PIC  9(009) COMP-3.
            10  A00-X1    PIC  X(008) VALUE '000000X1'.
            10  A00-RECORD-SEQ2 REDEFINES A00-X1   PIC  9(008).
            10  A00-RECORD-2    PIC  9(008) VALUE ZERO.
            10  A00-RECORD-3    PIC  9(009) COMP-3 VALUE ZERO.


           EXEC SQL INCLUDE SQLCA END-EXEC.

       PROCEDURE DIVISION.
      *    MOVE A00-RECORD-SEQ2 TO A00-RECORD-2.
      *    ADD +1 TO A00-RECORD-SEQ2.
      *    MOVE A00-RECORD-SEQ TO A00-RECORD-3.
      *    ADD +1 TO A00-RECORD-SEQ.
      *    if A00-RECORD-2 is numeric
      *    goback.
      ...
      ****** VERIFY TABLE (JUST TYPES AND LAB-TEST-ID)
       ...
       350-CHECK-LAB-TABLE.
           ...
           IF NOT VALID-CATEGORY IN TEST-CATEGORY(ROW-SUB)
      ...
      ******** Call to VSAM file to read record
           ...
           IF  DEFERRED
               AND NOT INPATIENT
               MOVE "*** INVALID OUTPAT. DEFERRED" TO
               ERR-MSG IN INPATIENT-TREATMENT-REC-ERR
               MOVE "Y" TO ERROR-FOUND-SW
               GO TO 500-EXIT.
      ...
      ******** EXEC SQL to get info from DB2
           MOVE DIAGNOSTIC-CODE-PRIMARY IN PATIENT-MASTER-REC TO
                DIAG-CD IN DCLDIAG-CODES.

      ****** CHECK FOR VALID DIAGNOSTIC CODE
           EXEC SQL
              SELECT DIAG_CODE INTO :DIAG-CD
              FROM DIAG_CODES
              WHERE DIAG_CODE = :DIAG-CD
           END-EXEC.
      ...
      ****** CHECK FOR VALID MEDICATION-ID
           MOVE MEDICATION-ID IN INPATIENT-TREATMENT-REC TO
                  MEDICATN-ID IN DCLMEDICATION.

           EXEC SQL
              DECLARE HOSP_C CURSOR FOR
                   SELECT * FROM DDS0001.HOSP_BED
                   ORDER BY 2
           END-EXEC.
           ...
           EXEC SQL
              SELECT MEDICATION_ID
                             INTO :MEDICATN-ID
              FROM MEDICATION
              WHERE MEDICATION_ID = :MEDICATN-ID
           END-EXEC.
      ...
      ****** CHECK FOR VALID SUPERVISOR NURSE-ID
           MOVE SUPERVISOR-NURSE-ID TO
                              SUPERVISE-NURSE-ID IN DCLWARD-DATA.
           EXEC SQL
      ...
      *    MOVE "T" TO RECORD-TYPE IN CUSTOMER-KEY.
           ADD +1 TO RECORDS-WRITTEN.
