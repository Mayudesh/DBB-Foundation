...
000200 PROGRAM-ID.  TRTMNTR.                                            00020002
000300 AUTHOR. JON SAYLES.                                              00030001
000400 INSTALLATION. COBOL DEV Center.                                  00040001
000500 DATE-WRITTEN. 02/23-18.                                          00050065
000600 DATE-COMPILED. 01/23/16.                                         00060065
...
002200******************************************************************00220001
002300                                                                  00230001
002400         INPUT FILE              -    TRMTDATA             00240001
002500                                                                  00250001
002600         VSAM MASTER FILE        -    PATMASTR             00260001
002700                                                                  00270001
002800         INPUT ERROR FILE        -    TRMTERR              00280001
002900                                                                  00290001
...
012500             '****** DUMP MSG1 ****************'.                 01250001
012600***************************************************************** 01260001
012700*    DUMP POINTER AREA1                                           01270001
...
016500     05  IN-RECORD-COUNT         PIC 9(10).                       01650001
...
019500                                                                  01950001
019600 77  WS-DATE                     PIC 9(6).                        01960001
019700                                                                  01970001
       01  INSPECT-TALLY-FIELDS.                                        01970122
ztrial      05 TALLY-1              PIC S9(05) COMP-3.                  01970249
ztrial      05 TALLY-2              PIC S9(05) COMP-3.                  01970349
...
020200     05 WS-MEDICATION-CHARGES    PIC S9(9)V99 COMP-3.             02020001
020300     05 WS-PHARMACY-CHARGES      PIC S9(6)V99 COMP-3.             02030054
020400     05 WS-ANCILLARY-CHARGES     PIC S9(4)V99 COMP-3.             02040054
020500                                                                  02050001
020600 01  MISC-WS-FLDS.                                                02060001
020700     05 STR-LTH                  PIC 9(04) VALUE 0.               02070001
020800     05 RETURN-CD                PIC S9(04) VALUE 0.              02080001
020900     05 ROW-SUB                  PIC S9(02) COMP-3.               02090054
                                                                        02090118
...
ztrial     INSPECT PRIMARY-DIAGNOSTIC-CODE                              03050768
ztrial     TALLYING TALLY-1  FOR LEADING 'DIAG'                         03050868
ztrial     TALLY-2  FOR LEADING 'DXX9'                                  03050968
ztrial     IF TALLY-1 NOT = 1 AND TALLY-2 NOT = 1                       03051068
ztrial        MOVE "*** INVALID DIAGNOSTIC-CODE-PRIMARY" TO             03051168
ztrial        ERR-MSG IN INPATIENT-TREATMENT-REC-ERR                    03051268
ztrial        MOVE "Y" TO ERROR-FOUND-SW                                03051368
ztrial        GO TO 300-EXIT.                                           03051468
ztrial                                                                  03060001
...
031000        MOVE "Y" TO ERROR-FOUND-SW                                03100068
...
036300*     CALL 'DATEVAL' USING TREATMENT-DATE, RETURN-CD.             03630001
...
040000     IF  (MEDICATION-COST > 995000                                04000049
040100     OR  MEDICATION-COST < 1.021)                                 04010049
...
041200         GO TO 400-EXIT.                                          04120001
041300                                                                  04130001
...
047800     MOVE "500-CROSS-FILE-EDITS" TO PARA-NAME.                    04780001
...
048700         GO TO 500-EXIT.                                          04870001
           MOVE +0 TO RETURN-CD.                                        04870101
                                                                        04870245
036100*     CALL WS-CALLED-MODULE USING TREATMENT-DATE, RETURN-CD.      04880101
                                                                        04880245
...
066300         GO TO 1000-DB2-ERROR-RTN.                                06630001
      ...
      ****                                                              06630902
      *     EXEC SQL                                                    06631002
      *        DECLARE C1 CURSOR FOR                                    06631102
      *        SELECT DEPT, MIN(PERF) AS "MIN(Perf)",                   06631202
      *                      MAX(PERF) AS "MAX(Perf)",                  06631302
      *                      AVG(PERF) AS "AVG(Perf)",                  06631402
      *                      MIN(HOURS) AS "MIN(Hours)",                06631502
      *                      MAX(HOURS) AS "MAX(Hours)",                06631602
      *                     DECIMAL (AVG(HOURS),5,2) AS "AVG(Hours)"    06631702
      *               FROM  EMPL E,  PAY P                06631802
      *               WHERE E.NBR = P.NBR                               06631902
      *               GROUP BY DEPT                                     06632002
      *       END-EXEC.                                                 06632102
                                                                        06632202
...
068800     DISPLAY '800-OPEN-FILES...'.                                 06880001
...
079900      DISPLAY 'SQLCODE ==> ' SQLCODE.                             07990001
            MOVE 70 TO SQLERRML.                                        07990553
            DISPLAY  'SQLERRMC: ' SQLERRMC.                             07990653
            DISPLAY 'SQLERRP: ' SQLERRP.                                07990753
            DISPLAY 'SQLERRD(3): ' SQLERRD(3)                           07990853
...
080400      GO TO 1000-ABEND-RTN.                                       08040001
       ...
       FORMAT-EXAMPLE.
      ** ztrial - unreachable code **
      ** ztrial - also, example of formatting source
      ** ztrial - so that the executasble logic matches
      ** ztrial - statement indentation(s)
      ...
      ******** Call to VSAM file to read record
           ...
           IF  LAB-TEST-ID(ROW-SUB) > "TRTRT0"
                          AND VALID-CATEGORY(ROW-SUB)
